...
public class P17478 {
	...
	static void message(int n) {	// n: 얼마나 반복되었는지
		String line = "";
		// 현재 반복된 수 만큼 밑줄의 길이가 길어지므로 n만큼 반복해준다
		for(int i = 0; i < n; i++) {
			line += "____";
		}
		System.out.println(line + "\"재귀함수가 뭔가요?\"");
		if(num == n) {	// num만큼 되었을 시 더이상 반복할 필요가 없기 때문에 return
	...
	public static void main(String[] args) {
		...
		message(0);	// 0부터 시작해서 num까지 반복하는 재귀함수
		sc.close();
